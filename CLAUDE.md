...
Key tables: `lnd_node_info`, `lnd_channels`, `lnd_peers`, `payments_inbound`, `payments_outbound`, `payments_forwarded`, `treasury_fee_policy`, `treasury_capital_policy`, `treasury_expansion_recommendations`, `treasury_expansion_executions`, `treasury_rebalance_costs`, `treasury_rebalance_executions`, `coinbase_onramp_sessions`, `contacts`, `member_keysend_status`, `network_payments`.
...
| `app/web/src/pages/Payments.tsx` | Network payments: Request Payment (QR + BOLT11), Pay Invoice (paste + decode + pay), Payment History table |
