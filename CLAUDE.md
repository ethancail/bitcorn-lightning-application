...
- **Public**: `/health`, `/api/node`, `/api/node/balances`, `/api/coinbase/onramp-url`, `/api/commodity-prices`, `/api/peers`, `/api/channels`, `/api/member/stats`, `POST /api/member/open-channel`, `/api/contacts`, `POST /api/contacts`, `PATCH /api/contacts/:pubkey`, `DELETE /api/contacts/:pubkey`, `POST /api/contacts/sync-peers`
