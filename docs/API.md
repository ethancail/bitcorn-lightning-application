...
| POST | `/api/treasury/rebalance/circular` | Treasury | Run a circular rebalance. Body: `tokens`, `max_fee_sats` (required); `outgoing_channel`, `incoming_channel` (optional). If channels omitted, the backend auto-selects the best donor (highest local ratio) and receiver (highest remote ratio) with different peers. Channel IDs may be ln-service format (`blockxindexxout`) or compact numeric. Response: `{ ok, rebalance: { ... } }` and, when auto-selected, `selected: { outgoing_channel, incoming_channel, outgoing_partner, incoming_partner, outgoing_score_ppm, incoming_score_ppm }`. |
