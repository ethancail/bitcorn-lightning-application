...
export const ENV = {
    ...
    // --- Loop (submarine swaps via Bitcorn's standalone loopd sidecar) ---
    // loopd runs as a dedicated service in Bitcorn's own compose, connected
    // straight to LND. Since v1.18.9 it is the Bitcorn-published image, because
    // upstream's arm64 manifest entry ships amd64 layers — app/loopd/Dockerfile
    // carries the mechanism and the ELF guard that enforces it.
    loopGrpcHost: process.env.LOOP_GRPC_HOST || "bitcorn-lightning-node_loopd_1",
    // gRPC port — loopd's --rpclisten port
    loopGrpcPort: Number(process.env.LOOP_GRPC_PORT ?? "11010"),
    // TLS certificate generated by loopd (under loopdir/<network>/)
    loopTlsCertPath: process.env.LOOP_TLS_CERT_PATH || "/loop-data/.loop/mainnet/tls.cert",
    // Loop macaroon for authentication (same directory as the cert)
