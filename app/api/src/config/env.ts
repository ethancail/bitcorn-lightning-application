...
export const ENV = {
    ...
    // gRPC hostname of the loopd sidecar (runs inside the Bitcorn app stack)
    loopGrpcHost: process.env.LOOP_GRPC_HOST || "bitcorn-lightning-node_loopd_1",
    // gRPC port — standalone loopd RPC listen port
    loopGrpcPort: Number(process.env.LOOP_GRPC_PORT ?? "11010"),
    // TLS certificate generated by loopd (in shared loop data volume)
    loopTlsCertPath: process.env.LOOP_TLS_CERT_PATH || "/loop-data/.loop/tls.cert",
    // Loop macaroon for authentication (in shared loop data volume)
